...
export default function Home() {
    ...
    const { isConnected, address } = useAccount({
        onConnect: async ({ address }) =>
            address && (await fundMyAccountOnLocalFork(address)),
    });
    const { chain } = useNetwork();
    const { openConnectModal, connectModalOpen } = useConnectModal();
    const { airdropContract, switchNetworkAsync, waitingForTransaction, error } =
        useContract({
            responseBytes,
            chain: CHAIN,
        });
    ...
    async function claimAirdrop() {
        ...
        try {
            ...
            const hash = await airdropContract.write.claimWithSismo([
                responseBytes,
                address,
            ]);
            ...
            <main className="main">
                ...
                {isConnected && (
                    <>
                        ...
                        {isConnected &&
                            !sismoConnectVerifiedResult?.amountClaimed &&
                            claimError && (
                                <>
                                    ...
                                    {claimError.slice(0, 50) ===
                                        ...
                                        <p style={{ color: "#0BDA51" }}>
                                            Please restart your frontend with "yarn dev"
                                            command and try again, it will automatically
                                            deploy a new contract for you!
                                        </p>
                                    )}
                        ...
                        {sismoConnectVerifiedResult?.verifiedAuths && (
                            <>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        {sismoConnectVerifiedResult.verifiedAuths.map(
                                            (auth, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        {getUserIdFromHex(
                                                            "0x" +
                                                                (
                                                                    auth.userId! as unknown as number
                                                                ).toString(16)
                                                        )}
                                                    </td>
                                                </tr>
                        ...
                        {sismoConnectVerifiedResult?.verifiedClaims && (
                            <>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        {sismoConnectVerifiedResult.verifiedClaims.map(
                                            (claim, index) => (
                                                <tr key={index}>
                                                    <td>
                                                        <a
                            ...
                            <tbody>
                                {CLAIMS.map((claim, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>
                                            {claim.isSelectableByUser ? "yes" : "no"}
                                        </td>
                                        <td>
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        {{ message: signMessage(address!) }.message}
                                    </td>
                                    <td>
                                        {sismoConnectVerifiedResult?.verifiedSignedMessage
                                            ? sismoConnectVerifiedResult.verifiedSignedMessage
